...
import { Supplier, AppData, PrintData } from '../../types';
...
export const SupplierModule = React.memo(({ data: globalData, onPrint }: { data: AppData, onPrint: (data: PrintData) => void }) => {
  ...
  const suppliers = useLiveQuery(async () => {
    let collection = local.suppliers.orderBy('name');
    
    if (searchTerm) {
      const lowerSearch = searchTerm.toLowerCase();
      return await local.suppliers
        .filter(s => 
  ...
  const selectedSupplier = useLiveQuery(
    () => selectedSupplierId ? local.suppliers.get(selectedSupplierId) : null,
    [selectedSupplierId]
  );
  ...
  const handleAutoSave = async (data: any) => {
    ...
    try {
      ...
      const supplierData: Supplier = {
        ...
        ledger: selectedSupplier?.ledger || []
      };

      await local.suppliers.put(supplierData);
      await syncService.queueChange('suppliers', 'SET', supplierData);
  ...
  const confirmDelete = async () => {
    if (deleteId) {
      try {
        await local.suppliers.delete(deleteId);
        await syncService.queueChange('suppliers', 'DELETE', { id: deleteId });
  ...
  const handlePrintLedger = () => {
    ...
    const printData: PrintData = {
      ...
      dateRange: `Supplier: ${selectedSupplier.name} | Generated on: ${format(new Date(), 'MMM dd, yyyy')}`,
      columns: [
      ...
      data: (selectedSupplier.ledger || []).map(entry => ({
      ...
      totals: [
        { label: 'Current Balance', value: `₹${(selectedSupplier.ledger?.[selectedSupplier.ledger.length - 1]?.balance || 0).toLocaleString()}`, isBold: true }
      ]
    };
  ...
  const SupplierRow = ({ index, style }: { index: number, style: React.CSSProperties }) => {
    ...
    return (
      <div 
        ...
        className={cn(
          ...
          selectedSupplierId === supplier.id && 'bg-indigo-50 border-l-4 border-indigo-600'
        )}
        onClick={() => setSelectedSupplierId(supplier.id)}
      >
        <div className="flex items-center justify-between mb-0.5">
          <h3 className="font-bold text-gray-900 truncate pr-2">{supplier.name}</h3>
          <span className="text-xs font-black text-indigo-600 whitespace-nowrap">
            ₹{(supplier.ledger?.[supplier.ledger.length - 1]?.balance || 0).toLocaleString()}
          </span>
        </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col">
      <ConfirmModal 
        ...
        message="Are you sure you want to delete this supplier? This will also remove their entire ledger history."
      />
      
      <div className="flex items-center justify-between no-print shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Suppliers</h1>
          <p className="text-sm text-gray-500">Manage vendors and track purchase ledgers</p>
        </div>
        ...
        <Card className="lg:col-span-1 p-0 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 min-h-0">
            {!suppliers ? (
              ...
              <div className="p-8 text-center text-gray-500 text-sm">No suppliers found</div>
            ) : (
              <List
                height={500}
                ...
                onItemsRendered={({ visibleStopIndex }) => {
                  if (visibleStopIndex >= suppliers.length - 5 && suppliers.length >= limit) {
          ...
          {selectedSupplier ? (
            <div className="space-y-6">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" onClick={() => handleEdit(selectedSupplier)} className="p-2">
                    <Edit2 size={16} />
                  </Button>
                  <Button variant="secondary" size="sm" onClick={() => setDeleteId(selectedSupplier.id)} className="p-2 text-red-600 hover:bg-red-50">
                    <Trash2 size={16} />
                  </Button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                  ...
                  <p className="text-xl font-black text-indigo-600">₹{(selectedSupplier.ledger?.[selectedSupplier.ledger.length - 1]?.balance || 0).toLocaleString()}</p>
                </div>
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Total Purchases</p>
                  <p className="text-xl font-black text-gray-900">₹{globalData.purchases.filter(p => p.supplierId === selectedSupplier.id).reduce((sum, p) => sum + p.totalAmount, 0).toLocaleString()}</p>
                </div>
              </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center py-20">
              <div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mb-4">
                <Truck className="text-indigo-200" size={40} />
              </div>
              <h3 className="text-lg font-bold text-gray-900">Select a supplier</h3>
              <p className="text-sm text-gray-500 max-w-xs">Choose a supplier from the list to view their full profile and ledger history.</p>
            </div>
          )}
        </Card>
      </div>

      <Modal isOpen={isAdding} onClose={() => setIsAdding(false)} title={editingId ? 'Edit Supplier' : 'Add New Supplier'}>
